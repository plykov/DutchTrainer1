...
const BANKS = {
  ...
  knm: { source: "content/sources/knm.json", output: "lib/content/knmItems.ts" },
};
...
const MC_RUNTIME_FIELDS = [
  "id", "examTrack", "cefrLevel", "skill", "taskType", "topic", "register", "regionVariant",
  "targetLemmas", "errorCodes", "l1ContrastFlag", "singleListen", "allowedTools", "timeLimitS",
  "responseMinLen", "difficultyTheta", "prompt", "options", "correctIndex", "explanationRu", "explanationEn",
];
...
function validateBank(document, expectedBank) {
  ...
  for (const item of document.items) {
    ...
    if (expectedBank === "speaking-exam") {
      ...
      for (const field of ["prompt", "sampleAnswer", "explanationRu", "explanationEn"]) {
        if (typeof item[field] !== "string" || !item[field].trim()) fail(`${label}: ${field} is required`);
      }
    } else if (expectedBank === "deelschrijftaken") {
      for (const field of ["instructionRu", "instructionEn", "taskPrompt"]) {
        if (typeof item[field] !== "string" || !item[field].trim()) fail(`${label}: ${field} is required`);
      }
    ...
    } else if (expectedBank === "knm") {
      for (const field of ["prompt", "explanationRu", "explanationEn", "topic"]) {
        if (typeof item[field] !== "string" || !item[field].trim()) fail(`${label}: ${field} is required`);
      }
      if (!Array.isArray(item.options) || item.options.length < 2 || item.options.some((value) => typeof value !== "string" || !value.trim())) {
        fail(`${label}: at least two non-empty options are required`);
      }
      if (!Number.isInteger(item.correctIndex) || item.correctIndex < 0 || item.correctIndex >= item.options.length) {
        fail(`${label}: correctIndex must identify an option`);
      }
      const fixed = { examTrack: "knm", cefrLevel: "A2", skill: "reading", taskType: "mc", register: "neutral", regionVariant: "netherlandic", l1ContrastFlag: false, singleListen: false, allowedTools: "none" };
      for (const [field, value] of Object.entries(fixed)) if (item[field] !== value) fail(`${label}: ${field} must be ${JSON.stringify(value)}`);
    }
...
function generateZinstaken(document, sourcePath) {
  ...
  const items = document.items
    .map(
      (item) =>
        `  { id: ${JSON.stringify(item.id)}, prompt: ${JSON.stringify(item.prompt)}, sampleAnswer: ${JSON.stringify(item.sampleAnswer)}, explanationRu: ${JSON.stringify(item.explanationRu)}, explanationEn: ${JSON.stringify(item.explanationEn)} },`,
    )
    .join("\n");
...
function generateDeelschrijftaken(document, sourcePath) {
  ...
  const items = document.items
    .map(
      (item) =>
        `  { id: ${JSON.stringify(item.id)}, instructionRu: ${JSON.stringify(item.instructionRu)}, instructionEn: ${JSON.stringify(item.instructionEn)}, taskPrompt: ${JSON.stringify(item.taskPrompt)}, fields: ${JSON.stringify(item.fields)} },`,
    )
    .join("\n");
...
function generateKnm(document, sourcePath) {
  const sourceLabel = path.relative(ROOT, sourcePath).replaceAll("\\", "/");
  const items = document.items.map((item) => {
    const runtime = Object.fromEntries(MC_RUNTIME_FIELDS.map((field) => [field, item[field]]));
    return `  ${JSON.stringify(runtime)},`;
  }).join("\n");
  return `// Generated by scripts/content-pipeline.mjs from ${sourceLabel}. Do not edit directly.\n\nimport { McItem } from "../types";\n\n// Original KNM demo questions. Source provenance and editorial review metadata\n// remain outside the learner bundle. These are not official exam questions.\nexport const KNM_ITEMS: McItem[] = [\n${items}\n];\n`;
}
...
function generateBank(bankName, document, sourcePath) {
  ...
  if (bankName === "knm") return generateKnm(document, sourcePath);
