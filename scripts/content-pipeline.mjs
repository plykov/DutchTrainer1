...
async function exportReview(bankName, requestedPath) {
  ...
  for (const item of document.items) {
    rows.push(
      [bankName, item.id, item.review.status, item.review.reviewer, item.review.reviewedAt, item.review.notes, item.text]
        .map(encodeTsv)
        .join("\t"),
    );
  }
...
async function importReview(bankName, requestedPath) {
  ...
  for (const item of loaded.document.items) {
    ...
    if (review.text !== item.text) fail(`Review TSV text for ${item.id} is stale or was edited`);
